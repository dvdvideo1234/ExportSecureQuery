  ...
  u8 CharIsDigit(s8 Ch)
  {
    return (Ch >= '0' && Ch <= '9');
  }

  u8 CharIsAlphaNum(s8 Ch)
  {
    if(CharIsDigit(Ch)){ return 0xFF; }
    return CharIsLetter(Ch);
  }

  u8 StrCppIsEmpty(void *Str)
  {
    u8* S = (u8*)Str;
    return (S[0] == 0);
  }

  u8 PointerIsNull(void *Str)
  {
    return (Str == NULL);
  }
  ...
  {
    if(PointerIsNull(S)){ return 0x00; }
    if(PointerIsNull((s8*)E)){ return 0x00; }
    if(StrCppIsEmpty(S)){ return 0x00; }
    if(StrCppIsEmpty((s8*)E)){ return 0x00; }
    u32 i = 0, j = 0;
    while(E[i] != '\0'){ i++; }
    while(S[j] != '\0'){ j++; }
    while(i+1)
  ...
  {
    if(PointerIsNull(Data)){ return 0; }
    u16 Rez = 0;
    Rez |= Data[0];
    Rez = Rez << 8;
    Rez |= Data[1];
  ...
  {
    if(PointerIsNull(Str)){ return 0; }
    if(PointerIsNull(Like)){ return 0; }
  ...
  {
    if(PointerIsNull(S)){ return NULL; }
    u32  i = 0;
  ...
  s8 *StrCppTrimLead(s8 *Str, s8 ch)
  {
    if(PointerIsNull(Str)){ return Str; }
    if(StrCppIsEmpty(Str)){ return Str; }
    u32 i = 0, j = 0;
    while(Str[i] == ch){ i++; }
    while(Str[i] != '\0')
    {
  ...
  s8 *StrCppTrimBack(s8 *Str, s8 ch)
  {
    if(PointerIsNull(Str)){ return Str; }
    if(StrCppIsEmpty(Str)){ return Str; }
    u32 i = StrCppOverallLen((u8*)Str) - 2;
    while(Str[i] == ch)
    {
      Str[i] = '\0';
      if(i == 0) break;
  ...
  s8 *StrCppTrimBoth(s8 *Str, s8 ch)
  {
    StrCppTrimBack(Str, ch);
    StrCppTrimLead(Str, ch);
    return Str;
  }

  s8 StrCppStartsLike(s8 *Str, const s8 *Like)
  {
    if(PointerIsNull(Str)){ return 0; }
    if(PointerIsNull((s8*)Like)){ return 0; }
  ...
  static s8 qStr[5] = {0};
  u8 StrCppIsNullSQL(s8 *Str)
  {
    if(PointerIsNull(Str)){ return 0x00; }
    u32 iLen = strlen(Str);
    if(iLen == 4)
    {
      strncpy(qStr, Str, sizeof(qStr));
      StrCppCapitalizeLatin(qStr);
      return (strcmp(qStr,"NULL") == 0) ? 0xFF : 0x00;
    }
    else if(iLen == 2)
    {
      return (strcmp(Str,"''") == 0) ? 0xFF : 0x00;
    }
    return 0x00;
  }
#endif
