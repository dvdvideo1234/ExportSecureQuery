...
s32 main(s32 argc, s8 *argv[])
{
  ...
  /*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
  FILE  *I=NULL, *O=NULL, *L=NULL, *P=NULL, *S=NULL;
  ...
  while(!feof(I))
  {
    ...
    StrCppTrimBoth(Buff,' ');
    if(!strncmp(Buff,"insert into ",12) || !strncmp(Buff,"INSERT INTO ",12))
    {
      bFound = 0xFF;
      i = 0;
      if(Dump)
      ...
      if(bFirstInsert)
      {
        ...
        while(Cnt < ParamCnt)
        {
          ...
          if((ValueName[Cnt] == NULL) || (FieldName[Cnt] == NULL))
          {
            ...
            while(Cnf < Cnt)
            {
              if(ValueName[Cnf]) free(ValueName[Cnf]);
              if(FieldName[Cnf]) free(FieldName[Cnf]);
              Cnf++;
            }
    ...
    else if(!strncmp(Buff,"values ",7) || !strncmp(Buff,"VALUES ",7))
    {
      FuncIndent = 0;
      StringIndent = 0;
      if(Dump)
